...
public abstract class ThriftCLIService extends AbstractService implements TCLIService.Iface, Runnable {
  ...
  public synchronized void init(HiveConf hiveConf) {
    ...
    String portString;
    LOG.info("Tien comment init param HiveConf");
  ...
  public synchronized void start() {
    super.start();
    LOG.info("Tien comment ThriftCLIService start");
  ...
  public int getPortNumber() {
    ...
    LOG.info("Tien comment ThriftCLIService getPortNumber");
    return portNum;
  }
  ...
  private String getUserName(TOpenSessionReq req) throws HiveSQLException {
    ...
    if (isKerberosAuthMode()) {
      ...
      LOG.info("Tien comment userName hiveAuthFactory: " + userName);
    }
    // Except kerberos, NOSASL
    if (userName == null) {
      userName = TSetIpAddressProcessor.getUserName();
      LOG.info("Tien comment userName TSetIpAddressProcessor: " + userName);
      LOG.info("Tien comment userName 111111111111: " + TSetIpAddressProcessor.getUserIpAddress());
    }
    // Http transport mode.
    ...
    if (cliService.getHiveConf().getVar(
      ...
      LOG.info("Tien comment userName HIVE_SERVER2_TRANSPORT_MODE: " + userName);
    ...
    LOG.info("Tien comment userName getShortName: " + userName);
  ...
  SessionHandle getSessionHandle(TOpenSessionReq req, TOpenSessionResp res)
    ...
    if (cliService.getHiveConf().getBoolVar(ConfVars.HIVE_SERVER2_ENABLE_DOAS) &&
        (userName != null)) {
      LOG.info("Tien comment HIVE_SERVER2_ENABLE_DOAS: " + cliService.getHiveConf().getBoolVar(ConfVars.HIVE_SERVER2_ENABLE_DOAS));
      ...
      LOG.info("Tien comment HIVE_SERVER2_ENABLE_DOAS: else");
